...
pub type Proof = VerkleProof<[u8; 32], LeafNodeWith32BytesValue<G1Affine>, G1Affine, IpaConfig<G1Affine>>;
...
impl VerkleStorage for Storage {
    fn new() -> Self {
        ...
        let tree: VerkleTreeWith32BytesKeyValue = VerkleTreeWith32BytesKeyValue::new(committer.clone());
    ...
    fn get<K: Clone + AsRef<[u8]>>(&self, _key: &K) -> Result<std::option::Option<&[u8; 32]>>  {
        ...
        let stored_value: Option<&[u8; 32]> = VerkleTreeWith32BytesKeyValue::get(&self.tree, &byte_key);
